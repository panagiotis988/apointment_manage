...
class Menu(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label_image.grid(row=1, column=3,padx=200, rowspan=6)
        ...
        self.MENU_CONFI_label = tk.LabelFrame(self, bd=2, bg='#335BA3')
        self.MENU_CONFI_label.grid(row=5, columnspan=2, padx=10, pady=10)
        self.MENU_CONFI_but = tk.Button(self.MENU_CONFI_label, command=lambda: controller.show_frame("MENU_CONFI"))

        label_entry5 = tk.LabelFrame(self, bd=1, bg='#969696')
        label_entry5.grid(row=6, columnspan=2, padx=2, pady=10)
        ...
        self.MENU_CONFI_but.grid(row=5, columnspan=2, padx=10, pady=10)
        exit_button.grid(row=6, columnspan=2, padx=5, pady=5)
        Menu.show_fr(self)

    def show_fr(self):
        email_state, html_state = picklel_helper.picklel_helper.load_confi()
        if email_state == True and html_state == True:
            self.MENU_CONFI_but.config(text="Ρυθμίσεις Email και Html", font=("Lucida", "14", "bold"),
                                       fg='#335BA3', bg='white', height=3, width=44)
        elif email_state == True and html_state == False:
            self.MENU_CONFI_but.config(text="Ρυθμίσεις Email και Html", font=("Lucida", "14", "bold"),
                                       fg='#335BA3', bg='orange', height=3, width=44)
        else:
            self.MENU_CONFI_but.config(text="Ρυθμίσεις Email και Html", font=("Lucida", "14", "bold"),
                                       fg='#335BA3', bg='red', height=3, width=44)
        self.after(10000, self.show_fr)
